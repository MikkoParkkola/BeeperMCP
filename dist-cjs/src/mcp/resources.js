...
const status_js_1 = require('../index/status.js');
const utils_js_1 = require('../../utils.js');
const routes = [];
let registered = false;
let logDbRef;
let logSecretRef;
function addResource(template, handler) {
  const keys = [];
...
function registerResources(logDb, logSecret) {
  ...
  addResource('im://matrix/room/:roomId/history', async (params, query) => {
    ...
    const out = (0, utils_js_1.queryLogs)(
      logDbRef,
      roomId,
      limit,
      from,
  ...
  addResource(
    ...
    async (params, query) => {
      ...
      const items = (0, utils_js_1.queryLogs)(
        logDbRef,
        params.roomId,
        before + after + 1,
        since,
  ...
  addResource('im://matrix/index/status', async (_params, _query, owner) => {
    return (0, status_js_1.indexStatus)(owner);
  });
}
function listResources() {
  return routes.map((r) => r.template);
}
async function handleResource(uri, query, owner) {
