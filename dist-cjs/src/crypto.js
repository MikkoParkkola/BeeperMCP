...
exports.loadOlm = loadOlm;
exports.loadRustCryptoAdapter = loadRustCryptoAdapter;
exports.loadFileCryptoStore = loadFileCryptoStore;
exports.restoreRoomKeys = restoreRoomKeys;
const fs_1 = __importDefault(require('fs'));
const path_1 = __importDefault(require('path'));
async function loadOlm(logger) {
  try {
...
async function loadRustCryptoAdapter(logger) {
  try {
    const mod = await Promise.resolve().then(() =>
      __importStar(require('@matrix-org/matrix-sdk-crypto-nodejs')),
    );
    logger.debug('rust-crypto adapter loaded');
    return mod.initRustCrypto;
  } catch (err) {
    logger.warn('rust-crypto adapter not available', err);
    return null;
  }
}
async function loadFileCryptoStore(cacheDir, logger) {
  try {
    const mod = await Promise.resolve().then(() =>
      __importStar(
        require('matrix-js-sdk/dist/cjs/crypto/node/crypto-store.js'),
      ),
    );
    logger.debug('FileCryptoStore loaded');
    const FileCryptoStoreClass = mod.FileCryptoStore;
    return new FileCryptoStoreClass(
      path_1.default.join(cacheDir, 'crypto-store'),
    );
  } catch (err) {
...
async function restoreRoomKeys(client, cacheDir, logger, recoveryKey) {
  if (
  ...
  try {
    const keyFile = path_1.default.join(cacheDir, 'room-keys.json');
