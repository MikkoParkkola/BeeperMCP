...
function truncateHistory(
  history,
  limits = { maxMessages: 20, maxChars: 16000 },
) {
...
async function* streamChat(p, model, messages) {
  if (p.type === 'openai') {
...
async function copyToClipboard(text) {
  // Try macOS pbcopy
  try {
    const r = (0, node_child_process_1.spawnSync)('pbcopy', [], {
      input: text,
    });
  ...
  // Try Linux xclip
  try {
    const r = (0, node_child_process_1.spawnSync)(
      'xclip',
      ['-selection', 'clipboard'],
      { input: text },
    );
