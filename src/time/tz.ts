...
export function toLocalKeys(instant: Date, tz: string): {
  tz_day: string;
  tz_week: number;
  tz_month: number;
  tz_year: number;
  ...
  let dt = DateTime.fromJSDate(instant, { zone: "utc" }).setZone(tz, { keepLocalTime: false });
  if (!dt.isValid && dt.invalidReason === "unsupported zone") {
    dt = DateTime.fromJSDate(instant, { zone: "utc" }).setZone(config.timezone.defaultTz);
  }
...
export function resolveNaturalRange(natural: string, clientTz: string): { fromUtc: Date; toUtc: Date } {
  ...
  return { fromUtc: from.setZone("utc").toJSDate(), toUtc: to.setZone("utc").toJSDate() };
}
