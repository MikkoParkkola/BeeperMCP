...
import { negotiateVersion, isStdioMode, shouldRequireApiKey } from './mcp-compat.js';
import type { MatrixClient } from 'matrix-js-sdk';

const version = process.env.npm_package_version || process.env.BEEPER_MCP_VERSION || 'dev';
...
export async function createMcpServer(
  ...
  const effectiveApiKey = apiKey || (isStdioMode() ? 'local-stdio-mode' : undefined);
...
export async function startStdioServer(
  ...
  const server = await createMcpServer(client, logDb, enableSend, 'local-stdio-mode', logSecret);
    ...
    if (transport.onMessage) {
      ...
      transport.onMessage = (message: any) => {
        if (message.method === 'initialize' && message.params?.protocolVersion) {
          ...
          console.error(`Protocol negotiation: client=${clientVersion}, server=${serverVersion}, using=${negotiated.version}`);
        }
...
export async function startHttpServer(
  ...
  const server = await createMcpServer(client, logDb, enableSend, apiKey, logSecret);
