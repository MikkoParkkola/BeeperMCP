...
export function buildMcpServer(
  ...
  if (enableSend) {
    ...
    srv.tool(
      ...
      authWrapper(async ({ room_id, draft_preview, send, persona_id }) => {
        // rate limit
        try {
          const { rateLimiter } = await import('./dist/src/security/rateLimit.js').catch(async () => await import('./src/security/rateLimit.js'));
          rateLimiter('mcp_tools_send', 10);
        } catch {}
        // guardrails
        try {
          const { checkGuardrails } = await import('./dist/src/security/guardrails.js').catch(async () => await import('./src/security/guardrails.js'));
          const guard = checkGuardrails(String(draft_preview || ''), persona_id);
          if (!guard.ok) {
            return { content: [{ type: 'json', json: { sent: false, reason: guard.reason } }] };
          }
        } catch {}
        if (!send) {
          return {
            content: [
              {
                ...
                json: {
                  ...
                  approvalForm: { room_id, draft_preview, persona_id: persona_id || null },
                },
              },
            ],
          };
        ...
        try {
          const { sanitizeText } = await import('./dist/src/security/sanitize.js').catch(async () => await import('./src/security/sanitize.js'));
  ...
  srv.tool(
    'inbox_list',
    z.object({ hours: z.number().int().positive().default(24), limit: z.number().int().positive().default(50), aliases: z.array(z.string()).default([]) }),
    authWrapper(async ({ hours, limit, aliases }) => {
      ...
      const looksActionable = (text) => {
        ...
        if (/(can you|could you|please|wdyt|what do you think|eta|any update)/i.test(t)) return true;
      ...
      for (const roomId of rooms) {
        ...
        for (let i = parsed.length - 1; i >= 0; i--) {
          ...
          const replied = parsed.slice(i + 1).some((q) => aliases.some((a) => a && q.sender.includes(a)));
          if (replied) continue;
          items.push({ id: `${roomId}:${p.ts}`, room_id: roomId, ts: p.ts, sender: p.sender, preview: p.text.slice(0, 120), status: 'open' });
          break;
        }
  ...
  srv.tool(
    ...
    authWrapper(async ({ room_id }) => {
      ...
      if (logDb) {
        ...
        const rows = logDb
          .prepare('SELECT line FROM logs WHERE room_id = ? AND ts >= ? ORDER BY ts ASC')
          .all(room_id, since);
      ...
      let out = { language: null, style_hints: [], audience_notes: [], sensitivities: [], updated_at: new Date().toISOString() };
      try {
        const { sample } = await import('./dist/src/mcp/sampling.js').catch(async () => await import('./src/mcp/sampling.js'));
        const resp = await sample({ prompt, maxTokens: 384 });
        const raw = resp.text || '';
        const lang = (raw.match(/LANG:\s*(.+)/i) || [])[1]?.trim();
        const sec = (name) => raw.split(new RegExp(`\n${name}:\n`, 'i'))[1]?.split(/\n[A-Z]+:/)[0] || '';
        const bullets = (s) => Array.from(s.matchAll(/^\-\s+(.+)$/gim)).map((m) => m[1].trim());
      ...
      const saved = { ...out, room_id, updated_at: new Date().toISOString(), _ts: Date.now() };
  ...
  srv.tool(
    ...
    z.object({
      room_id: z.string(),
      source: z.object({ text: z.string().optional(), event_id: z.string().optional() }),
    ...
    authWrapper(async ({ room_id, source, intention, extra_instructions, tone, language, to }) => {
      let context = '';
      if (source?.text) context = source.text;
      else if (source?.event_id && logDb) {
        const row = logDb.prepare('SELECT line FROM logs WHERE event_id = ?').get(source.event_id);
        context = row?.line || '';
      }
      const briefRes = await srv.callTool('brief_room', { room_id }).catch(() => null);
      const brief = briefRes?.content?.[0]?.json || {};
      const lang = language || brief.language || 'the same language as the context';
      ...
      return { content: [{ type: 'json', json: { drafts: text.split(/\n\s*\n/).slice(0, 2) } }] };
    }),
  );
  ...
  srv.tool(
    'revise_reply',
    z.object({ room_id: z.string(), base_draft: z.string(), extra_instructions: z.string() }),
  ...
  srv.tool(
    'qa',
    z.object({ question: z.string(), rooms: z.array(z.string()).optional(), limit: z.number().int().positive().default(3) }),
    authWrapper(async ({ question, rooms, limit }) => {
      const highlights = [];
      let ctx = '';
      if (logDb) {
        const rs = rooms && rooms.length
          ? rooms
          : logDb.prepare('SELECT DISTINCT room_id FROM logs ORDER BY room_id').all().map((r) => r.room_id);
        const kw = String(question).toLowerCase().split(/\W+/).filter(Boolean);
        for (const roomId of rs) {
          const rows = logDb
            .prepare('SELECT ts, line, event_id FROM logs WHERE room_id = ? ORDER BY ts DESC LIMIT 400')
            .all(roomId);
          for (const r of rows) {
            const line = String(r.line || '');
            const t = line.toLowerCase();
            if (kw.some((w) => t.includes(w))) {
              highlights.push({ room_id: roomId, uri: `im://matrix/room/${roomId}/message/${r.event_id || ''}/context`, snippet: line });
              ctx += `${line}\n`;
  ...
  srv.tool(
    'digest_generate',
    z.object({ rooms: z.array(z.string()).optional(), hours: z.number().int().positive().default(24) }),
    authWrapper(async ({ rooms, hours }) => {
      const os = await import('os');
      const path = await import('path');
      const fs = await import('fs');
      const home = process.env.BEEPERMCP_HOME || path.join(os.homedir(), '.BeeperMCP');
      const outDir = path.join(home, 'digests');
      let linesByRoom = {};
      let roomList = rooms && rooms.length ? rooms : [];
      if (logDb) {
        if (!roomList.length) {
          roomList = logDb.prepare('SELECT DISTINCT room_id FROM logs ORDER BY room_id').all().map((r) => r.room_id);
        }
        const since = new Date(Date.now() - hours * 3600_000).toISOString();
        for (const roomId of roomList) {
          const rows = logDb
            .prepare('SELECT ts, line FROM logs WHERE room_id = ? AND ts >= ? ORDER BY ts ASC LIMIT 800')
            .all(roomId, since);
          linesByRoom[roomId] = rows.map((r) => r.line);
        }
      }
      const stats = Object.entries(linesByRoom).map(([roomId, lines]) => `${roomId}: ${lines.length} lines`).join('\n');
      ...
      try { fs.writeFileSync(file, `# Digest (last ${hours}h)\n\n${markdown}\n`, { mode: 0o600 }); } catch {}
      return { content: [{ type: 'json', json: { file_uri: file, markdown, preview: markdown.slice(0, 1000) } }] };
    }),
  );
  ...
  srv.tool(
    ...
    authWrapper(async ({ text, target_language }) => {
      ...
      return { content: [{ type: 'json', json: { translated_text: out, target_language: target } }] };
    }),
  );
  ...
  srv.tool(
    'tone_learn',
    z.object({ aliases: z.array(z.string()), since_days: z.number().int().positive().optional(), per_person_max: z.number().int().positive().optional() }),
    authWrapper(async ({ aliases, since_days, per_person_max }) => {
      try {
        const eng = await import('./dist/src/style/engine.js').catch(async () => await import('./src/style/engine.js'));
        const map = eng.learnPersonalTone?.(aliases, { sinceDays: since_days, perPersonMax: per_person_max }) || {};
        const count = Object.keys(map).length;
        return { content: [{ type: 'json', json: { profiles: map, count } }] };
      } catch (e) {
        return { content: [{ type: 'json', json: { profiles: {}, error: String(e) } }] };
      }
    }),
  );
  ...
  srv.tool(
    ...
    authWrapper(async ({ person_id }) => {
      try {
        ...
        return { content: [{ type: 'json', json: { profile: null, error: String(e) } }] };
      }
    }),
  );
