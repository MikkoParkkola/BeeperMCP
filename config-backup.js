...
function saveConfig() {
  ...
  console.log(
    '💡 You can restore this config on another machine using: node config-backup.js restore',
  );
}
...
function restoreConfig() {
  ...
  try {
    ...
    for (const [filename, content] of Object.entries(backup.files)) {
      ...
      if (fs.existsSync(filepath)) {
        console.log(
          `⚠️  ${filename} already exists. Backing up as ${filename}.backup`,
        );
    ...
    console.log(
      '⚠️  Remember to update your MATRIX_TOKEN in .beeper-mcp-server.env',
    );
    console.log(
      '💡 Run npm install and npm run build to ensure dependencies are ready',
    );
  } catch (error) {
...
function showUsage() {
  ...
  console.log(
    'This utility helps you backup and restore your BeeperMCP configuration',
  );
